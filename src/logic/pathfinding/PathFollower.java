...
public class PathFollower {
    ...
    public boolean moveToDestination(UnitData unitData, PathNode nextNode, Api api) {
        ...
        if (atDestination(unitData, nextNode.x, nextNode.y)) {
            api.setThrustSpeed(unitData.id, ThrustSpeed.NONE);
        ...
        if (moveForward) {
            ...
            if (unitData.rotation != Rotation.NONE) {
                api.setRotationSpeed(unitData.id, Rotation.NONE);
            }
            if (unitData.thrustSpeed != ThrustSpeed.FORWARD) {
                api.setThrustSpeed(unitData.id, ThrustSpeed.FORWARD);
            }
        }
        else {
            // Stop moving forward
            if (unitData.thrustSpeed != ThrustSpeed.NONE) {
                api.setThrustSpeed(unitData.id, ThrustSpeed.NONE);
            }
            ...
            if (slowSpeed) {
                if (angle < 0) {
                    if (unitData.rotation != Rotation.SLOW_RIGHT) {
                        api.setRotationSpeed(unitData.id, Rotation.SLOW_RIGHT);
                    }
                } else {
                    if (unitData.rotation != Rotation.SLOW_LEFT) {
                        api.setRotationSpeed(unitData.id, Rotation.SLOW_LEFT);
                    }
                }
            }
            else {
                if (angle < 0) {
                    if (unitData.rotation != Rotation.RIGHT) {
                        api.setRotationSpeed(unitData.id, Rotation.RIGHT);
                    }
                } else {
                    if (unitData.rotation != Rotation.LEFT) {
                        api.setRotationSpeed(unitData.id, Rotation.LEFT);
                    }
                }
            }
        }
    ...
    public boolean atDestination(UnitData unitData, int destX, int destY) {
        return Math.abs(destX - unitData.x) < ALLOWED_DESTINATION_OFFSET &&
                Math.abs(destY - unitData.y) < ALLOWED_DESTINATION_OFFSET;
    }
    ...
    public float angleBetweenUnitAndPoint(UnitData unitData, int x, int y) {
        ...
        unitToDest.sub(unitData.x, unitData.y);
        ...
        float angle = unitToDest.angle() - unitData.orientation;
